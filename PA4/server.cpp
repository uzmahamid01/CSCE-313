...
int main (int argc, char* argv[]) {
	...
	cout << "_port_no in server: " << port << endl;
	...
	//call constructor to create a server-side control channel

    TCPRequestChannel* control_channel = new TCPRequestChannel("", port);

	while (true) {
		//accept a new connection socket
		int new_sock = control_channel->accept_conn();
		
		if (new_sock == -1) {
			cerr << "Error accepting connection in server: " << strerror(errno) << endl;
			continue;  
		}

		//create a new channel with accepted socket(the socket is already established)
		TCPRequestChannel* new_channel = new TCPRequestChannel(new_sock);

		//dispatch a new thread to handle_process_loop
		//Since the main thread is not blocked, it can accept more connections (use dispatch instead of join)
